...
The submitted per-seed and summary CSV/JSON files are the active evidence for the report's architecture-selection table. The optional source notebook is included and can be rerun if full executed-notebook provenance for this specific run is required.
